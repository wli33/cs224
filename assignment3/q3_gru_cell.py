...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_z = tf.get_variable('Wz',[self.state_size,self.state_size],
                                  initializer = tf.contrib.layers.xavier_initializer())
            U_z = tf.get_variable('Uz',[self.input_size,self.state_size],
                                  initializer = tf.contrib.layers.xavier_initializer())
            b_z = tf.get_variable('bz', [self.state_size],initializer = tf.zeros_initializer())

            W_r = tf.get_variable('Wr',[self.state_size,self.state_size],
                                  initializer = tf.contrib.layers.xavier_initializer())
            U_r = tf.get_variable('Ur',[self.input_size,self.state_size],
                                  initializer = tf.contrib.layers.xavier_initializer())
            b_r = tf.get_variable('br', [self.state_size],initializer = tf.zeros_initializer())
            
            W_o = tf.get_variable('Wo',[self.state_size,self.state_size],
                                  initializer = tf.contrib.layers.xavier_initializer())
            U_o = tf.get_variable('Uo',[self.input_size,self.state_size],
                                  initializer = tf.contrib.layers.xavier_initializer())
            b_o = tf.get_variable('bo', [self.state_size],initializer = tf.zeros_initializer())

            z_t = tf.sigmoid(tf.matmul(inputs,U_z)+tf.matmul(state,W_z)+b_z)
            r_t = tf.sigmoid(tf.matmul(inputs,U_r)+tf.matmul(state,W_r)+b_r)
            o_t = tf.tanh(tf.matmul(inputs,U_z)+ r_t * tf.matmul(state,W_z)+b_z)

            news_state = z_t * state + (1 - z_t) * o_t
