...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        paddinglength = max_length - sen_len
        filled_sentence = sentence + ( [zero_vector] * paddinglength ) if paddinglength>0 else sentence[:max_length]
        filled_labels = labels + ( [zero_label] * add_length) if paddinglength>0 else labels[:max_length]
        masks = [True] * sen_len + [False] * paddinglength if padding length>0 else [True] * max_length
        
        ret.append((filled_sentence,filled_labels,masks))
...
class RNNModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.mask_placeholder = tf.placeholder(tf.bool, shape= (None,self.max_length), name = 'masks')
    ...
    def add_prediction_op(self):
        ...
        state = tf.zeros([input_shape[0],Config.hidden_size],name = 'state')
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                output, state = RNNCell(x[:,time_step, :],state,"RNN")
                output = tf.nn.dropout(output,dropout_rate)
                pred.append(tf.matmul(output,U)+b2)
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels = mask_labels,logits = mask_preds)
