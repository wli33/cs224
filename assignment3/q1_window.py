...
def make_windowed_data(data, start, end, window_size=1):
    ...
    ### YOUR CODE HERE (5-20 lines)
       ...
       l = 0
       for i in range(window_size, len_sen + window_size):
           temp_feats = []
           for j in range(i - window_size,i + window_size+1):
               temp_feats.extend(sentence[j])
           window_data.append(temp_feats,label[l])
           l += 1           
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.labels_placeholder = tf.placeholder(tf.int32,(None,),name = 'label')
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=1):
        ...
        if labels_batch is not None:
            feed_dict[self.dropout_placeholder] = labels_batch
    ...
    def add_embedding(self):
        ...
        embedding = tf.nn.embedding_lookup(embedding, self.input_placeholder)
        embedding = tf.reshape(embediing,[-1,self.config.n_window_features * self.config.embedding_size])
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable('W',shape=[self.config.n_window_features * self.config.embedding_size,self.config.hidden_size],
