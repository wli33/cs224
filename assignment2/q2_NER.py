...
class NERModel(LanguageModel):
  ...
  def add_embedding(self):
    ...
    with tf.device('/cpu:0'):
      ...
      with tf.variable_scope("embedding_layer") as scope:
        ...
        window = tf.reshape(window,[-1,self.config.window_size*self.config.embed_size],name = "window")
  ...
  def add_loss_op(self, y):
    ...
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(label = self.labels_placeholder,logits = y))
