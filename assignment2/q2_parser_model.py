...
class ParserModel(Model):
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=1, beta_regul=10e-7):
        ...
        """
        ### YOUR CODE HERE
        feed_dict = {self.input_placeholder:inputs_batch,self.beta_regul:beta_regul,
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope("model"):
            self.W = xavier_initializer((self.config.n_features * self.config.embed_size, self.config.hidden_size))
    ...
    def add_training_op(self, loss):
        ...
        train_op = tf.train.AamOptimizer(self.config.lr).minimize(loss)
