...
class SoftmaxModel(Model):
  ...
  def add_model(self, input_data):
    ...
    with tf.name_scope("model"):
      W = tf.Variable(tf.random_normal([self.config.n_features,self.config.n_classes],stddev=0.01),name="weights")
...
##      W = tf.get_variable('weights',[n_features,n_classes],initializer=tf.random_normal_initializer())
##      b = tf.get_variable('biaes',[n_classes],initializer=tf.zeros_initializer())
##
##      out = tf.matmul(input_data,W)+b
##      out = softmax(out)
    ### END YOUR CODE
